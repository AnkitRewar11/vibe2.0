...
function ImageSettingSection({ selectedEl }: Props) {
    ...
    const saveUploadedFile = async () => {
        ...
        try {
            ...
            // If the upload returns a URL, update the preview
            if (imageRef && (imageRef as any).url) {
                setPreview((imageRef as any).url);
            }
        } catch (err) {
    ...
    const GenerateAiImage = ()=>{
        ...
        const url =`https://ik.imagekit.io/sai5uodrwj/ik-genimg-prompt-${altText}/${Date.now()}.png?tr=`
        setPreview(url);
